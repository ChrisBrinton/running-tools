...
class AudioEngine: AudioEngineProtocol {
    ...
    func setup() throws {
        ...
        self.emphasisBeatBuffer = try makeEmphasisBeatBuffer(format: format)
        self.silentBuffer = try makeSilentBuffer(format: format)
    }
    ...
    private func bufferForBeat() -> AVAudioPCMBuffer? {
        ...
        if isEmphasisBeat {
            ...
            // Neither - return nil (silence, but we still need to schedule for timing)
            return nil
        }
    }
    ...
    private func scheduleBeats(player: AVAudioPlayerNode, buffer: AVAudioPCMBuffer, bpm: Int, count: Int) {
        ...
        guard let nodeTime = player.lastRenderTime,
            ...
            // Only add completion handler to the last beat to continue scheduling
            for i in 0..<count {
        ...
        for i in 0..<count {
            ...
            // Only add completion handler to the LAST beat to avoid multiple scheduling chains
            let isLastBeat = (i == count - 1)
            if isLastBeat {
                // Use the beat buffer if available, otherwise use silent buffer for timing
                let bufferToSchedule = beatBuffer ?? silentBuffer ?? buffer
                player.scheduleBuffer(bufferToSchedule, at: time, options: []) { [weak self] in
                    // When last beat plays, schedule next batch (continuous playback)
                    self?.scheduleNextBeat(player: player, buffer: buffer, bpm: bpm)
                }
            } else {
                if let buf = beatBuffer {
    ...
    func makeEmphasisBeatBuffer(format: AVAudioFormat) throws -> AVAudioPCMBuffer {
        ...
        let samples = Self.generateEmphasisBeatSamples(
            sampleRate: sampleRate,
            duration: emphasisBeatDuration,
            frequency: emphasisBeatFrequency,
            amplitude: emphasisBeatAmplitude
        )
    ...
    func makeSilentBuffer(format: AVAudioFormat) throws -> AVAudioPCMBuffer {
        let frameCount = AVAudioFrameCount(sampleRate * beatDuration)
    ...
    /// Generates samples for emphasis beat - higher pitched snap/click
    static func generateEmphasisBeatSamples(
        sampleRate: Double,
        duration: Double,
        frequency: Float,
        amplitude: Float
        ...
        for frame in 0..<totalFrames {
            ...
            let envelope = emphasisEnvelopeValue(frame: frame, totalFrames: totalFrames)
    ...
    private static func emphasisEnvelopeValue(frame: Int, totalFrames: Int) -> Float {
    ...
    func updateBeatFrequency(isTooSlow: Bool) {
        ...
        do {
            beatBuffer = try makeBeatBuffer(format: format)
        } catch {
