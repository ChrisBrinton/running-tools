...
/// - HealthKit: Workout session and sample recording (unless in companion mode)
...
class WorkoutManager: NSObject, WorkoutManagerProtocol {
    ...
    func startWorkout(with configuration: RunConfiguration) throws {
        ...
        let gpsSettings = settingsProvider()
        if gpsSettings.gpsFilterDebugSounds, let gps = gpsManager as? GPSManager {
    ...
    private func handleLocationUpdate() {
        ...
        // Handle auto-end after state is published
        if shouldAutoEnd {
            handleAutoEnd()
        }
    }
    ...
    private func handlePaceUpdate(_ pace: Pace?) {
        ...
        if state.isInGracePeriod {
            ...
            // Priority: slow (master) > medium > fast with smart filtering
